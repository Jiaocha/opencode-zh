...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const commands = [
    ...
    {
      name: "diff.collapse",
      title: t("tui.diff.collapse_item"),
      category: "VCS",
      run: focusRunner({
        files() {
          const highlighted = highlightedFileNode()
    ...
    },
    {
      ...
      title: t("tui.diff.previous_file"),
      category: "VCS",
      run() {
        jumpRelativePatchFile(-1)
      },
    },
    {
      ...
      title: t("tui.diff.mark_reviewed"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: t("tui.diff.toggle_file_tree"),
      category: "VCS",
      run() {
    ...
    },
    {
      ...
      title: t("tui.diff.switch_source"),
      category: "VCS",
      run() {
  ...
  const switchDiffOptions = createMemo(() => [
    {
      title: t("tui.diff.working_tree"),
      value: "git" as const,
      description: t("tui.diff.show_current_git_changes"),
    },
    {
      title: t("tui.diff.last_turn"),
      value: "last-turn" as const,
      description: t("tui.diff.show_last_assistant_turn"),
    },
  ])
  ...
  const openSwitchDiffDialog = () => {
    props.api.ui.dialog.replace(() => (
      <DialogSelect
        title={t("tui.diff.switch_source_short")}
        skipFilter={true}
        renderFilter={false}
        current={mode()}
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: t("tui.diff.move_down") },
      { key: "k,up", cmd: "diff.up", desc: t("tui.diff.move_up") },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: t("tui.diff.page_down") },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: t("tui.diff.page_up") },
      { key: "m", cmd: "diff.mark_reviewed", desc: t("tui.diff.mark_reviewed_action") },
    ...
    <box position="absolute" zIndex={2500} left={0} top={0} width={dimensions().width} height={dimensions().height}>
      <PanelGroup axis="y" width="100%" height="100%">
        <Panel border="none" flexShrink={0} padding={0} paddingLeft={1}>
          <text fg={theme().text}>{t("tui.diff.title")} </text>
          <text fg={theme().textMuted}>
            {mode() === "last-turn" ? t("tui.diff.last_turn") : t("tui.diff.working_tree")}
          </text>
          <box flexGrow={1} />
          <text fg={theme().textMuted}>
            {files().length} {t("tui.diff.files")}
          </text>
        </Panel>
          ...
          <Switch>
            <Match when={diff.loading}>
              ...
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{t("tui.diff.loading")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && files().length === 0}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>{t("tui.session.no_diff")}</text>
              </box>
            </Match>
            <Match when={!diff.loading && diff.error}>
              <Separator axis="x" />
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().error}>{t("tui.diff.load_failed")}</text>
              </box>
            </Match>
            <Match when={!diff.loading}>
              <PanelGroup axis="x">
                        ...
                        return (
                          <box ref={(element: BoxRenderable) => registerPatchNode(entry.fileIndex, element)}>
                            ...
                            <Show
                              ...
                              fallback={<text fg={theme().textMuted}>{t("tui.diff.no_patch")}</text>}
                            >
                              {(patch) => (
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          <Show when={switchFocusShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.focus_file_tree")}</span>
              </text>
            )}
          </Show>
          <Show when={nextFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.next_file_short")}</span>
              </text>
            )}
          </Show>
          <Show when={previousFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.previous_file_short")}</span>
              </text>
            )}
          </Show>
          <Show when={switchSourceShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.switch_source_short")}</span>
              </text>
            )}
          </Show>
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.mark_reviewed_short")}</span>
              </text>
            )}
          </Show>
          <Show when={helpShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>{t("tui.diff.all_shortcuts")}</span>
              </text>
            )}
          </Show>
        </Panel>
...
function DiffViewerHelpDialog() {
  ...
  const rows = [
    {
      shortcut: () => "q",
      action: t("tui.diff.close_viewer"),
      description: t("tui.diff.quit_viewer"),
    },
    {
      shortcut: useCommandShortcut("diff.switch_focus"),
      action: t("tui.diff.focus_file_tree_action"),
      description: t("tui.diff.focus_between_tree_and_patch"),
    },
    {
      shortcut: useCommandShortcut("diff.next_file"),
      action: t("tui.diff.next_file_action"),
      description: t("tui.diff.next_file_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.previous_file"),
      action: t("tui.diff.previous_file_action"),
      description: t("tui.diff.previous_file_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.toggle_file_tree"),
      action: t("tui.diff.toggle_file_tree_action"),
      description: t("tui.diff.toggle_file_tree_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.single_patch"),
      action: t("tui.diff.toggle_patches"),
      description: t("tui.diff.toggle_patches_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.switch_source"),
      action: t("tui.diff.change_source"),
      description: t("tui.diff.change_source_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.toggle_view"),
      action: t("tui.diff.change_layout"),
      description: t("tui.diff.change_layout_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.expand_all"),
      action: t("tui.diff.expand_all_folders"),
      description: t("tui.diff.expand_all_folders_desc"),
    },
    {
      shortcut: useCommandShortcut("diff.mark_reviewed"),
      action: t("tui.diff.mark_reviewed_action"),
      description: t("tui.diff.mark_reviewed_desc"),
    },
  ]
  ...
  return (
    <box paddingLeft={2} paddingRight={2} paddingBottom={1} gap={1}>
      ...
      <box flexDirection="row">
        ...
        <text fg={theme.textMuted}>{t("tui.diff.description")}</text>
      </box>
      <For each={rows}>
        {(row) => (
          <box flexDirection="row">
